...
double CILQRConstraintManager::LambdaProjection(const double & lambda){
    ...
    if (lambda > config_ptr_->MaxLambda)
    {
        lambda_projection = config_ptr_->MaxLambda;
    }
    else if (lambda < config_ptr_->MinLambda)
    {
        lambda_projection = config_ptr_->MinLambda;
    }
    else
    {
        lambda_projection = lambda;
    }
    return lambda_projection;

}
double CILQRConstraintManager::ComputeStepAugmentedCost(Eigen::VectorXd state_vec, Eigen::VectorXd input_vec, int8_t step){
    double cost = 0.0;
    if (step < config_ptr_->horizon)
    {
        for (int i = 0; i < constraint_list_.size(); i++)
        {
            double constraint_cost = constraint_list_[i]->ComputeConstraint(state_vec, input_vec, step);
